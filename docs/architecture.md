...
`library_identity` 的单例记录由迁移为每个数据库生成独立的固定 `LibraryId`，数据库备份恢复会保留该身份，客户端显示名称、API 地址与 MinIO 配置变化不影响它。文件和文件夹的位置使用 `MF1:<LibraryId>:<asset|folder>:<对象 Guid 或 root>`，不携带凭据或临时下载 URL。根目录仅允许 `folder:root`。

`LocationService` 在服务端解析定位码、完整逻辑路径和名称片段，所有查询复用目录查看权限。定位码优先于分享文本中的路径快照，移动和重命名不改变对象 ID；错误、未知版本或其他库的定位码不会降级到路径匹配。完整路径只有唯一精确匹配才自动定位，同名文件/文件夹或模糊匹配返回最多 50 个候选供用户选择。不可访问与不存在使用相同响应；已删除对象只在通过权限校验的精确定位中返回回收站状态。名称搜索不返回回收站内容。

客户端将位置复制为可读文本与定位码，可直接粘贴到搜索框或“粘贴定位”对话框。跳转时重新校验目标位置和权限、清除遮挡目标的筛选，并单独读取目标文件以覆盖列表前 200 条之外的情况。其他库的定位请求可暂存到当前会话，在用户自行配置连接并登录后继续解析；不会按分享文本自动连接任意服务器。第一版不注册操作系统外链协议。
