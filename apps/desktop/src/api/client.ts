...
export const createApi = (
  ...
  return {
    ...
    location: (kind: LocationKind, id: string | null) => request<AssetLocation>(`/api/locations/${kind}${id ? `/${id}` : ''}`),
    resolveLocation: (input: string) => request<LocationResult>('/api/locations/resolve', { method: 'POST', body: JSON.stringify({ input }) }),
    ...
    updateAccount: (id: string, userName: string, displayName: string, isEnabled: boolean) => request<Account>(`/api/admin/accounts/${id}`, { method: 'PUT', body: JSON.stringify({ userName, displayName, isEnabled }) }),
    deleteAccount: (id: string) => request<void>(`/api/admin/accounts/${id}`, { method: 'DELETE' }),
