...
function FolderBranch({ folder, level, selectedId, onSelect, onMenu }: { folder: Folder; level: number; selectedId: string | null; onSelect: (id: string) => void; onMenu: (folder: Folder, anchor: HTMLElement) => void }) {
  const [open, setOpen] = useState(true)
  const hasChildren = Boolean(folder.children?.length)
  return <>
    <div className={`tree-row ${selectedId === folder.id ? 'is-selected' : ''}`} style={{ '--tree-level': level } as React.CSSProperties}>
      <button className="tree-row__chevron" disabled={!hasChildren} onClick={() => setOpen(value => !value)} aria-label={open ? '折叠' : '展开'}>
      ...
      {folder.canOperate ? <button className="tree-row__menu" onClick={event => onMenu(folder, event.currentTarget)} aria-label={`${folder.name} 操作`}><MoreHorizontal /></button> : null}
    </div>
    {open ? folder.children?.map(child => <FolderBranch key={child.id} folder={child} level={level + 1} selectedId={selectedId} onSelect={onSelect} onMenu={onMenu} />) : null}
  </>
}

export function FolderTree({ folders, selectedId, trash, operations, accounts, isMasterAdmin, onSelect, onTrash, onOperations, onAccounts, onFolderMenu }: Props) {
  return (
    <nav className="folder-navigation" aria-label="资产目录">
      ...
      <div className="folder-tree">{folders.map(folder => <FolderBranch key={folder.id} folder={folder} level={0} selectedId={selectedId} onSelect={onSelect} onMenu={onFolderMenu} />)}</div>
